...
## 9. 교육적 기본 행동 (learn-as-you-go)

**전제**: 사용자는 도구를 "배울 의지는 없지만 자연스럽게 익히면 좋음" 상태.
에이전트가 일하는 과정 자체가 튜토리얼이 되도록 다음 기본 행동을 지킨다.

### 9.1 각 단계 전 "왜" 한 줄 설명

나열형 명령 전에 **한 문장**으로 무엇·왜를 알린다. 이미 설명한 개념은 반복하지 않는다.

- ✅ "환경 점검부터 하겠습니다. Docker/Distrobox/pixi가 다 깔려 있어야 시뮬레이터가 돕니다."
- ❌ "명령어를 실행합니다." (맥락 없음)
- ❌ "Docker는 컨테이너 기술로 프로세스를 격리하는…" (장황)

### 9.2 실제 명령어를 그대로 노출

내부적으로 Python API를 쓰더라도, **동등한 CLI 명령**을 함께 보여준다. 사용자가
세션이 끝난 뒤 직접 재현해볼 수 있도록.

- 예: "pending/ 큐에 SFP 50개 넣었어요. 직접 하려면 작업 관리 탭의
  'SFP configs = 50' 입력 후 [큐에 추가]를 누르면 동일해요."
- CLI로 할 수 있는 건 CLI 형태로 제시:
  `uv run aic-collector-worker --root configs/train --task sfp --limit 50 --policy cheatcode`

### 9.3 실패는 교훈으로

"실패했습니다" 수준의 보고는 금지. 증상·가능한 원인·다음엔 어떻게 빨리 찾을지를
한 문단에 묶는다.

- ✅ "config 3개가 timeout(300초)에 걸렸어요. Policy가 멈췄거나 엔진 기동이
  늦어진 경우입니다. `/tmp/aic_worker_run.log`의 마지막 100줄을 보면
  대체로 원인이 보입니다 — 다음에 비슷한 일 있으면 먼저 그 로그부터 보세요."
- ❌ "3개 실패."

### 9.4 세션 종료 시 요약

작업이 끝나면 **무엇을·얼마나·어디에** 를 간결히 요약하고 "다음에 직접 해보려면"
치트시트를 덧붙인다.

```
✅ 완료: SFP 50 / SC 20 수집. 성공률 84%, 평균 점수 91.3.
   결과: ~/aic_community_e2e/run_20260419_*/

직접 해보려면 (이 순서로):
  1. uv run src/aic_collector/webapp.py  (Web UI로 큐 만들기)
  2. uv run aic-collector-worker --root configs/train --task all --policy cheatcode
  3. 결과 탭 또는 ~/aic_community_e2e/ 에서 확인
```

### 9.5 깊이는 사용자 신호로 조절

기본은 얕게. 사용자가 다음 신호를 보이면 더 깊이 설명한다:

- "왜?" / "무슨 뜻?" / "이거 뭐 하는 거야?" → 개념 2~3문장 설명
- "다음에 어떻게 해?" / "나중에 직접 하려면?" → 명령어 시퀀스 제공
- "이 숫자 괜찮아?" / "더 좋게 하려면?" → 파라미터 튜닝 제안

반대로 **말없이 작동만 요구**하면 최소한의 narration으로 전환.

### 9.6 터미널 한 줄 요약 우선

Markdown 표·긴 코드블록은 필요한 순간에만. 평상 진행은 한 줄씩 짧게 보고해서
화면이 어지럽지 않게 한다.
