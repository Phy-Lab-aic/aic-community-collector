...
- 파일명: `config_{task}_{NNNNNN}.yaml` (NNNNNN = 6자리 sample_index)
- append 모드: 모든 상태 디렉토리를 스캔해 **max(NNNNNN) + 1** 부터 번호 부여 → 중복 없음
- Seed는 `base_seed + sample_index` 로 파생되므로 재실행해도 동일 config가 나옵니다

### 👥 Team Mode

`configs/team/preset.yaml`이 존재하면 작업 관리 탭은 자동으로 team mode로 전환됩니다. 이 모드에서는 작업을 손으로 자유롭게 조정하는 대신, preset이 팀 단위 규칙을 강제합니다.

- `Member` 선택기를 먼저 고릅니다.
- `start_index`와 randomization 파라미터는 preset에서 내려오며 잠깁니다.
- team mode는 현재 **SFP config 생성만 지원**합니다. `SFP configs` 수량만 정한 뒤 `📁 큐에 추가 (...)` 버튼을 누르면 됩니다. `SC configs` 생성은 team mode에서 지원하지 않습니다.
- `큐 루트`는 각 사용자의 **로컬 출력 위치**만 정합니다. 슬롯 예약은 로컬 queue root와 무관하게 전역 `configs/team/seed_ledger.yaml` 기준으로 처리됩니다.
- 각 submit은 `configs/team/seed_ledger.yaml`에 새 claim을 append합니다.
...
20 Hz 기준은 낮추지 않습니다. 저사양 PC에서는 불필요한 로컬 부하를 줄이는 최소 조합을 사용하세요.

```bash
uv run aic-prefect-run --config configs/e2e_low_spec_20hz.yaml
```

큐 워커로 돌릴 때는 같은 원칙을 CLI 옵션으로 맞춥니다.
...
uv run aic-collector-worker \
  ...
  --collect-episode false \
  --headless \
  --timeout 300 \
  --recover
```

핵심은 `headless=true`, `use_compressed=false`, `collect_episode=false`입니다. 이 조합은 Gazebo/RViz GUI, image_transport republish, policy-side PNG/npy 저장을 피하고 raw camera MCAP을 20 Hz로 직접 기록합니다. 단, raw Image 토픽은 디스크 쓰기량이 크므로 SSD 여유 공간과 쓰기 성능은 필요합니다.
